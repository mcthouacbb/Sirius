...
UCI::UCI()
	: m_State(CommState::IDLE), m_Search(m_Board), m_InputQueue(shouldQuit)
{

}

void UCI::setToFen(const char* fen)
{
	m_PrevStates.clear();
	m_PrevMoves.clear();
...
void UCI::goCommand(std::istringstream& stream)
{
	...
	m_Search.iterDeepUCI(limits, *this);
	if (m_State == CommState::QUITTING)
		return;
	m_State = CommState::IDLE;

	std::cout << "bestmove " << comm::convMoveToPCN(currMove) << std::endl;
}
