...
int Search::search(SearchThread& thread, int depth, SearchStack* stack, int alpha, int beta, bool pvNode, bool cutnode)
{
    ...
    if (!pvNode && !inCheck && !excluded)
    {
        ...
        bool noTTCutnode = cutnode && !ttHit;
        int rfpMargin = (improving ? rfpImpMargin : rfpNonImpMargin) * depth - 20 * noTTCutnode - rfpOppWorsening * oppWorsening + (stack - 1)->histScore / rfpHistDivisor;
