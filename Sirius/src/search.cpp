...
int Search::search(SearchThread& thread, int depth, SearchStack* stack, int alpha, int beta, bool pvNode, bool cutnode)
{
    ...
    while ((scoredMove = ordering.selectMove()).score != MoveOrdering::NO_MOVE)
    {
        ...
        if (doSE)
        {
            int sBeta = std::max(-SCORE_MATE, ttData.score - (sBetaScale + 20 * (ttPV && !pvNode)) * depth / 16);
        ...
        if (score > bestScore)
        {
            ...
            if (bestScore >= beta)
            {
                ...
                // history(~527 elo)
                int histDepth = depth + (stack->staticEval < beta - 30) + (bestScore > beta + histBetaMargin);
