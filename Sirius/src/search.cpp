    ...
    bool pvNode, bool cutnode)
{
    ...
    while ((scoredMove = ordering.selectMove()).score != MoveOrdering::NO_MOVE)
    {
            ...
            && moveScore <= MoveOrdering::FIRST_KILLER_SCORE)
        {
            ...
            reduction -= lmrCorrplexity * (std::abs(stack->eval - rawStaticEval) > lmrCorrplexityMargin);
            reduction += lmrCutnode * cutnode;
            reduction += lmrFailHighCount * ((stack + 1)->failHighCount >= static_cast<uint32_t>(lmrFailHighCountMargin));
