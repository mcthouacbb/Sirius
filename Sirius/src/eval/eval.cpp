...
void initEvalData(const Board& board, EvalData& evalData, const PawnStructure& pawnStructure)
{
    ...
    evalData.kingRing[WHITE] &= ~(attacks::pawnEastAttacks<WHITE>(wpawns) & attacks::pawnWestAttacks<WHITE>(wpawns));
    ...
    evalData.kingRing[BLACK] &= ~(attacks::pawnEastAttacks<BLACK>(bpawns) & attacks::pawnWestAttacks<BLACK>(bpawns));
}
